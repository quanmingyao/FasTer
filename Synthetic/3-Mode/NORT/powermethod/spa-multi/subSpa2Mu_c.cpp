...
// (subs, vals, U, szTen)
{
    ...
    // output matrix
	pargout[0] = mxCreateDoubleMatrix((mwSize)iK, (mwSize)(iI1*iI3), mxREAL);
	...
	for (size_t i = 0; i != iNnz; i++)
	{
		...
		const size_t ipi = (iSub3 - 1) + iI3*(iSub1 - 1);
		const double* pui = pU + (iSub2 - 1);
		double* pxi = pX + iK*ipi;
		
		for (size_t k = 0; k != iK; k++)
		{
			pxi[k] = pxi[k] + (*ipVals) * pui[k*iI2];
